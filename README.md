...
| [Self Hosted Login Page](/self-hosted) | An application server that uses the Okta Sign-In Widget on a custom login page within the application. The widget uses the interact code flow to obtain the token and login the user, then creates a cookie session for the user in the Spring application. | Traditional web applications with a custom login page and server-side rendered pages. |
